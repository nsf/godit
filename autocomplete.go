...
func test_ac_func(path string, r io.Reader, cursor cursor_location_ex) ([]ac_proposal, int) {
	testdata_display := []string{
	...
	ps := []ac_proposal(nil)
	for i := 0; i < len(testdata_content); i++ {
		ps = append(ps, ac_proposal{
			display: []byte(testdata_display[i]),
			content: []byte(testdata_content[i]),
		})
	}
	return ps, 3
}
